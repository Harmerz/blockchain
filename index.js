...
// PERSON
app.get('/person/:address', async(req, res) => {   //http://localhost:3000/products/
    try {
...
// INVENTORY
app.get('/inventory/:address', async(req, res) => {   //http://localhost:3000/products/
  try {
...
app.post('/inventory/:address', async(req, res) => {   //http://localhost:3000/products/
    try {
        const address = req.params.address;
...
app.post('/send', async (req, res) => {
    try {
        ...
        const tx = await supplyChain.sendShipment(userAddress1, userAddress2, weight, productId, productRecords)
        await tx.wait();
        res.json({success: true})
...
// SHIPMENT RECORDS
app.get('/shipment/:address', async(req, res) => {   //http://localhost:3000/products/
    try {
        const address = req.params.address;
...
app.post('/change/:address', async (req, res) => {
    try {
       ...
       const tx = await supplyChain.processMyProduct(address, productId, newProductId, weight, productRecords);
        await tx.wait();
        res.json({success: true})
...
// FINAL PRODUCT
app.get('/final/:id', async(req, res) => {   //http://localhost:3000/products/
    try {
        const id = req.params.id;
...
app.get('/final/:address', async(req, res) => {   //http://localhost:3000/products/
    try {
        const address = req.params.address;
        const shipments = await supplyChain.getSendShipmentByAddress(address);
        let myShipments = []
...
// ORIGIN PRODUCT
app.get('/origin/:id', async(req, res) => {   //http://localhost:3000/products/
    try {
        const id = req.params.id;
        const shipments = await supplyChain.getShipmentById(id);
        let myShipments = []
